...
const ListUser = ({ dsChuyenNganh }) => {
    ...
    const handleDeleteChuyenNganh = (id) => {
        ...
        api({ type: TypeHTTP.DELETE, sendToken: true, path: `/chuyennganh/delete/${id}` })
            .then(res => {
                    ...
                    {dsChuyenNganh.map((chuyenNganh, index) => (
                        <tr key={index} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                            ...
                            <td className="px-6 py-4 flex items-center gap-1">
                                ...
                                <button onClick={() => handleDeleteChuyenNganh(chuyenNganh._id)} className='px-4 py-1 rounded-md text-[14px] bg-[red] text-white'>Xóa</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
