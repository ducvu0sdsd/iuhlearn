...
const FormYeuCauThemTiet = ({ data, hidden }) => {
    ...
    const handleSubmit = () => {
        if (!phongSelected) {
            globalHandler.notify(notifyType.WARNING, 'Vui lòng chọn phòng')
            return
        }
        if (currentDate === '') {
        ...
        const body = {
            ...data, status: 'ACCEPTED', response: {
                message: `Đã thêm tiết học mới vào ngày ${currentDate} tiết ${tietSelected.value} tại ${phongSelected.tenPhong}`,
                data: {
                    ...data.request.data,
                    phong: {
                        ...phongSelected,
                    ...
                    tiet: tietSelected.value,
                    ngay: tietSelected.ngay,
                    newDate: {
    ...
    useEffect(() => {
        if (phongSelected) {
            const arr = JSON.parse(JSON.stringify(dsThoiGianPhongDaDangKy))
            const filter = arr.filter(item => (item.phong.maPhong === phongSelected._id))
            setDsThoiGianPhongDaDangKyFilter(filter)
        }
    }, [phongSelected])
                ...
                <div className='w-full h-full flex'>
                    <div style={{ transition: '0.5s', marginLeft: `-${step * 100}%` }} className='w-[100%] flex'>
                        <div className='min-w-[100%] p-4 flex flex-col items-center justify-start gap-1'>
                            ...
                            <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder='Vui lòng điền lý do khiến bạn từ chối yêu cầu (nếu từ chối)' className='border-[1px] px-2 py-2 text-[13px] border-[#dcdcdc] h-[150px] mt-1 focus:outline-0 w-[100%] rounded-lg' />
                            <div className='w-full flex justify-end items-center gap-1 text-[12px] mt-2'>
                                ...
                                <button onClick={() => setStep(1)} className='bg-[#4ec782] text-[white] px-2 py-[4px] rounded-md transition-all hover:scale-[1.05]'>Thêm tiết</button>
                            </div>
                        </div>
                        <div className='min-w-[100%] p-4 flex flex-col items-center justify-start gap-1'>
                            <span className='w-full text-[15px] font-semibold'>Chọn Phòng</span>
                            <div className='w-full h-[80%] overflow-y-auto grid grid-cols-6 gap-2'>
                                {phongs.map((phong, index) => {
                                    if (phong.loai === data.request.data.loaiPhong) {
                                        return <div onClick={() => setPhongSelected(phong)} style={{ backgroundColor: phongSelected?._id === phong._id ? '#83efb1' : '#ebebeb' }} className='transition-all hover:scale-[1.03] cursor-pointer rounded-lg w-full flex items-center justify-center flex-col aspect-square' key={index}>
                            ...
                            <div className='w-full flex justify-end items-center gap-1 text-[12px] mt-1'>
                                ...
                                <button onClick={() => setStep(2)} className='bg-[#4ec782] text-[white] px-2 py-[4px] rounded-md transition-all hover:scale-[1.05]'>Tiếp tục</button>
                            </div>
                        </div>
                        <div className='min-w-[100%] p-4 flex flex-col items-center justify-start gap-1'>
                            <span className='w-full text-[15px] font-semibold'>Chọn Thời Gian</span>
                            ...
                            <div className='w-full flex justify-end items-center gap-1 text-[12px] mt-1'>
                                <button onClick={() => setStep(1)} className='bg-[#999] text-[white] px-2 py-[4px] rounded-md transition-all hover:scale-[1.05]'>Hủy</button>
                                <button onClick={() => handleSubmit()} className='bg-[#4ec782] text-[white] px-2 py-[4px] rounded-md transition-all hover:scale-[1.05]'>Xác nhận</button>
                            </div>
                        </div>
                    </div>
                </div>
...
export default FormYeuCauThemTiet